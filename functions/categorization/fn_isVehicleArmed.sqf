...
// if (DMORBAT_debug) then { diag_log format ["DMORBAT: _parents: %1", _parents] };

_checkMags = {
    params ["_uniqueMags"];
    // if (DMORBAT_debug) then { diag_log format ["DMORBAT: _uniqueMags: %1", _uniqueMags] };
    ...
    private _checkAmmoParent = true;
    {
        ...
        // if (DMORBAT_debug) then { diag_log format ["DMORBAT: _mag: %1", _x] };
        private _ammo = getText (configfile >> "CfgMagazines" >> _x >> "ammo");
        // if (DMORBAT_debug) then { diag_log format ["DMORBAT: _ammo: %1", _ammo] };
        private _ammoParents = [configFile >> "CfgAmmo" >> _ammo, false] call BIS_fnc_returnParents;
        // if (DMORBAT_debug) then { diag_log format ["DMORBAT: _ammoParents: %1", _ammoParents] };    
        _checkAmmoParent = true;
        {
            if (_armed || !_checkAmmoParent) exitWith { [_armed, _checkAmmoParent] };
            private _hitStr = str (_x >> "hit");
            // if (DMORBAT_debug) then { diag_log format ["DMORBAT: _hitStr: %1", _hitStr] };
            _checkAmmoParent = if (_hitStr == "") then { true } else { false };
            private _hit = getNumber (_x >> "hit");
            // if (DMORBAT_debug) then { diag_log format ["DMORBAT: _ammo: %1 _hit: %2", _x, _hit] };
...
// Check main weapons
// if (DMORBAT_debug) then { diag_log format ["DMORBAT: isVehicleArmed -- Checking main weapons of %1", _class] };
private _armed = false;
private _checkAmmoParent = true;
{
    if (_armed || !_checkAmmoParent) exitWith { true };
    // if (DMORBAT_debug) then { diag_log format ["DMORBAT: _class: %1", _x] };
...
if (!_armed) then {
    // if (DMORBAT_debug) then { diag_log format ["DMORBAT: isVehicleArmed -- Checking turrets of %1", _class] };
    // Check turrets
    private _turretsWithMags = -1;
    {
        ...
        // if (DMORBAT_debug) then { diag_log format ["DMORBAT: _class: %1", _x] };
        {
            ...
            _turretsWithMags = 0;
            {
                ...
                // if (DMORBAT_debug) then { diag_log format ["DMORBAT: turretConfig: %1", _x] };
                private _mags = getArray (_x >> "magazines");
                private _uniqueMags = [];
                { _uniqueMags pushBackUnique _x} forEach _mags;
                if (count _uniqueMags > 0) then { _turretsWithMags = _turretsWithMags + 1; };
                // if (DMORBAT_debug) then { diag_log format ["DMORBAT: _turretsWithMags: %1", _turretsWithMags] };
                private _checkMags = [_uniqueMags] call _checkMags;
                _armed = _checkMags select 0;
                _checkAmmoParent = _checkMags select 1;
