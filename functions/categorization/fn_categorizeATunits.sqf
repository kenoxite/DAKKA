...
if (DAKKA_debug) then { diag_log format ["DAKKA: categorizeATunits - Categorizing AT units..."] };
...
if (count _riflemenAT == count _properUnitClassNames) then {
    ...
    if (DAKKA_debug) then { diag_log format ["DAKKA: categorizeATunits - AT units follow proper suffixes. Categorizing..."] };
    ...
    private _ATremove = [];
    {
        ...
        {
            private _parents = [configFile >> "CfgWeapons" >> _x, true] call BIS_fnc_returnParents;
            // if (DAKKA_debug) then { diag_log format ["DAKKA: categorizeATunits - _weapon: %1, _parents: %2", _x, _parents] };
            if ("Launcher" in _parents) exitWith {
                _usedLauncher = _x;
    ...
    // if (DAKKA_debug) then { diag_log format ["DAKKA: categorizeATunits - _riflemenAT (after removing): %1, removed: %2", _riflemenAT, _ATremove] };
    // if (DAKKA_debug) then { diag_log format ["DAKKA: categorizeATunits - _ATlaunchers: %1", _ATlaunchers] };
    ...
    // if (DAKKA_debug) then { diag_log format ["DAKKA: categorizeATunits - _launcher; %1, _launcherMags: %2", _launcher, _launcherMags] };
        ...
        if (_fakeMags) then {
            ...
            // if (DAKKA_debug) then { diag_log format ["DAKKA: categorizeATunits - _launcher; %1", _launcher] };
    ...
    // if (DAKKA_debug) then { diag_log format ["DAKKA: categorizeATunits - _launcher: %1, _usedMags: %2", _launcher, _usedMags] };
    ...
    // if (DAKKA_debug) then { diag_log format ["DAKKA: categorizeATunits - _ATlaunchers (dmg): %1", _ATlaunchers] };

    // Sort from greater to lower damage
    _ATlaunchers sort false;
    if (DAKKA_debug) then { diag_log format ["DAKKA: categorizeATunits - _ATlaunchers (sorted): %1", _ATlaunchers] };
    ...
    // if (DAKKA_debug) then { diag_log format ["DAKKA: categorizeATunits - _ATammount: %1, _ATdmgCut: %2, _ATdmgMean: %3", _ATammount, _ATdmgCut, _ATdmgMean] };
    ...
    if (DAKKA_debug) then { diag_log format ["DAKKA: categorizeATunits - _LATlaunchers: %1", _LATlaunchers] };
    if (DAKKA_debug) then { diag_log format ["DAKKA: categorizeATunits - _HATlaunchers: %1", _HATlaunchers] };
    ...
    private _unitLauncher = [];
    {
        ...
        // if (DAKKA_debug) then { diag_log format ["DAKKA: categorizeATunits - _unitLauncher: %1", _unitLauncher] };
