...
{
    // if (DAKKA_debug) then { diag_log format ["DAKKA: _eligibleATAll (pre-filter) %1: %2", _forEachIndex, _x select 0] };
} forEach _eligibleATAll;
// Pick a group with at least 2 AT
{  
    private _ATcount = 0;
    private _units = _x select 0;
    // if (DAKKA_debug) then { diag_log format ["DAKKA: Play Now - AT team check - _units: %1",_units] };
    {
        private _unit = _x;
        private _roles = [[_unit]] call DAKKA_fnc_groupRoles;
        if (_roles select 0) then { _ATcount = _ATcount + 1 };
        // if (DAKKA_debug) then { diag_log format ["DAKKA: Play Now - AT team check - _unit: %1 isAT: %2", _unit, _roles select 0] };
    } forEach _units;
    _ATgroupsCount pushBack [_forEachIndex, _ATcount];
} forEach _eligibleATAll;
// if (DAKKA_debug) then { diag_log format ["DAKKA: Play Now - AT team check - _ATgroupsCount: %1",_ATgroupsCount] };
...
{
    if (DAKKA_debug) then { diag_log format ["DAKKA: _eligibleATAll (post-filter) %1: %2", _forEachIndex, _x select 0] };
} forEach _eligibleATAll;
