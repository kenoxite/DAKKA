...
diag_log format ["DAKKA: Assigning new group to %1", p1];  
_grp = grpNull; 
...

{
  ...
  _unit = if (_isMan) then {
            ([_unitClass, _pos, if (isNull _grp) then { west } else {_grp}, [], 0, "NONE", true] call DAKKA_fnc_spawnMan);
          } else {
            _isAir = [_unitClass] call DAKKA_fnc_isAir;
            ([_unitClass, _pos, if (isNull _grp) then { west } else {_grp}, [], 200, if (_isAir) then { "FLY" } else { "NONE" }, true] call DAKKA_fnc_spawnVehicle);
          };
  ...
  if (count _unitLoadout > 0) then {
    if ([_forEachIndex] call DAKKA_fnc_checkIfSelIsPlayer) then {
        _applyLoadout = false;
    };
  };
    ...
    if (!_isAir) then {
        ...
        if ((count _nearTerrObj) > 0) then {
          ...
          if (count _emptyPos > 0) then {
            diag_log format ["DAKKA: setPlayerGroup - FOUND safe position for %1 (%2)", _unit, _unitClass];
            _unit setPos _emptyPos;
          } else {
            diag_log format ["DAKKA: setPlayerGroup - NOT FOUND safe position for %1 (%2)", _unit, _unitClass];
          };
        };
    };
  };
...
if (pitch p1 == 0) then {
    p1 setPitch 1;
    diag_log format ["DAKKA: setPlayerGroup Player cloned badly. Something's fishy with the cloned identity... Trying again.", ""];
    [_playableUnit, p1] call DAKKA_fnc_cloneUnit;
};

// Apply loadout
if (DAKKA_debug) then { diag_log format ["DAKKA: setPlayerGroup _playerLoadout: %1", _playerLoadout] };
...
// Move passengers to vehicles
{
    ...
    for [{private _i = 0}, {_i < count _passengerSeats && (count _groupPassengers) > 0}, {_i = _i + 1}] do {
        ...
        if (DAKKA_debug) then { diag_log format ["DAKKA: setPlayerGroup - %1 is moving into %2", _groupPassengers select 0, typeOf _x] };
