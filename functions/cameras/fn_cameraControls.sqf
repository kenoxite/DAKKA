...
if (_action == "leftBtnMouse") then {
    ...
    // systemChat format ["DMORBAT_cameraZoom: %1", DMORBAT_cameraZoom];
    // _screenPosX = (_screenPos select 0) - (DMORBAT_cameraZoom);
    // _screenPosY = (_screenPos select 1) - (DMORBAT_cameraZoom);
    ...
    // DMORBAT_cameraTargetPos = screenToWorld _screenPos;
};
...
if (_action == "ZOOMIN" && DMORBAT_cameraZoom > 0.01) then {
	DMORBAT_cameraZoom = DMORBAT_cameraZoom - 0.02;
};

if (_action == "ZOOMOUT" && DMORBAT_cameraZoom < 2) then {
	DMORBAT_cameraZoom = DMORBAT_cameraZoom + 0.02;
};

if (_action == "LEFT") then {
	DMORBAT_cameraX = DMORBAT_cameraX - 0.05;
};
...
if (_action == "UP") then {
	DMORBAT_cameraY = DMORBAT_cameraY - 0.05;
};
