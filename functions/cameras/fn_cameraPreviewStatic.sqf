...
if (DAKKA_debug) then { diag_log format ["DMBORBAT: camerapreviewstatic _target: %1", _target] };
...
DAKKA_cameraTargetPos = _targetPos;
DAKKA_cameraRelPos = _relPos;
...
while { DAKKA_previewCameraPlaying && !isNull _camera } do {
	...
	if(!isNull _camera && DAKKA_previewCameraPlaying) then {
		_camera camPrepareFOV DAKKA_cameraZoom;
        // _camera camSetRelPos DAKKA_cameraRelPos;
		_camera camSetTarget [(DAKKA_cameraTargetPos select 0) - (_relX - DAKKA_cameraX), DAKKA_cameraTargetPos select 1, (DAKKA_cameraTargetPos select 2) + (_height - DAKKA_cameraY)];
		// _camera camSetPos [(_pos select 0) + DAKKA_cameraX, (_pos select 1) + DAKKA_cameraY, (_pos select 2)];
