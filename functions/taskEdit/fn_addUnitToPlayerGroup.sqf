...
if (isNull _unit) exitWith { systemChat "DAKKA: --- ERROR --- No unit was selected!" };
_realUnit = [DAKKA_previewGroup, 0] call DAKKA_fnc_realUnitbyIndex;
// if (DAKKA_debug) then { diag_log format ["DAKKA: addUnitToPlayerGroup _realUnit: %1", _realUnit] };
_unitClass = (typeOf _realUnit);
// if (DAKKA_debug) then { diag_log format ["DAKKA: addUnitToPlayerGroup _unitClass: %1", _unitClass] };

_taskData = DAKKA_TaskData select (DAKKA_Task - 1);
_playerGroupData = [_taskData, "Player group"] call BIS_fnc_getFromPairs; 
if (DAKKA_debug) then { diag_log format ["DAKKA: addUnitToPlayerGroup _playerGroupData: %1", _playerGroupData] };
// Check for the unit mod dependencies
_mod = [_unitClass] call DAKKA_fnc_modsCheck;
// if (DAKKA_debug) then { diag_log format ["DAKKA: addUnitToPlayerGroup _knownMods: %1", _knownMods] };
...
if (!([_unitClass] call DAKKA_fnc_isMan) && [_index] call DAKKA_fnc_checkIfSelIsPlayer) then {
  // if (DAKKA_debug) then { diag_log format ["DAKKA: addUnitToPlayerGroup adding unit: %1", _realUnit] };
  [IDC_COMBO_TASK_GROUPS_CREW, _realUnit] call DAKKA_fnc_updateCrewSlotsCombo;
} else {
