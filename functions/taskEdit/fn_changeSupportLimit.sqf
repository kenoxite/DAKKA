...
if (_catIndex < 0) exitWith { diag_log format ["DAKKA: changeSupportLimit Suppport type ""%1"" not found!", _supportType]; false };
...
if (DAKKA_debug) then { diag_log format ["DAKKA: changeSupportLimit _supportLimit: %1", _supportLimit] };
