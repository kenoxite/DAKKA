// INIT 4
// ENEMY GROUPS CREATION

#include "..\control_defines.hpp";
#define CURRENTPAGE 4

disableSerialization;

//Reset map things
// if (!isNil "DMORBAT_EH_mapSingleClick") then { removeMissionEventHandler ["MapSingleClick", DMORBAT_EH_mapSingleClick] }; 
...
_groupsData = [_taskData, "Enemy groups"] call BIS_fnc_getFromPairs;
...
[IDC_COMBO_FACTIONS, false] call DMORBAT_fnc_updateFactionCombo;
_ctrl ctrlSetEventHandler ["LBSelChanged", '[IDC_COMBO_FACTIONS, _this select 1, false, true] call DMORBAT_fnc_ComboFactions_selChanged;'];
...
_ctrl ctrlSetEventHandler ["TreeSelChanged", ' [_this select 1] call DMORBAT_fnc_TreeFactionGroups_selChanged; '];

	// Buttons
	_ctrl = (_display displayCtrl IDC_BT_ADD_GROUP);
	_ctrl ctrlSetText "Add group to Enemy Groups";
	_ctrl ctrlSetEventHandler ["ButtonClick", '[] call DMORBAT_fnc_addGroupToGroup; '];
	if (DMORBAT_Task == 2) then {
		_ctrl ctrlSetTooltip "Automatically adds the group to its corresponding enemy group area";
	} else {
		_ctrl ctrlSetTooltip "Adds the unit to the selected enemy group";
	};
...
_ctrl ctrlSetEventHandler ["TreeSelChanged", ' [_this select 1, IDC_TREE_FACTION_GROUPS, IDC_TREE_FACTION_UNITS] call DMORBAT_fnc_TreeFactionUnits_selChanged; '];
	...
	_ctrl ctrlSetText "Add Unit to Enemy Groups";
	_ctrl ctrlSetEventHandler ["ButtonClick", '[] call DMORBAT_fnc_addUnitToGroup; '];
	_ctrl ctrlSetTooltip "Adds the unit to the selected enemy group";
	if (DMORBAT_Task == 2) then {
...
_ctrl ctrlShow false;
	...
	_grp1Name = format ["Enemy %1", (_groupsData select 0) select 0];
	_ctrl = (_display displayCtrl IDC_TITLE_GROUP1);
	_ctrl ctrlSetText toUpper (_grp1Name);

	_ctrl = (_display displayCtrl IDC_TREE_GRP1);
	[IDC_TREE_GRP1, 1] call DMORBAT_fnc_updateCustomGroupsTreeList;
	_ctrl ctrlSetEventHandler ["TreeSelChanged", ' [IDC_TREE_GRP1, _this select 1, 1, true, [IDC_BT_1_GRP1, IDC_BT_2_GRP1, IDC_BT_3_GRP1]] call DMORBAT_fnc_TreeCustomGroup_selChanged; '];
	_ctrl ctrlShow true;
		...
		_ctrl ctrlSetEventHandler ["ButtonClick", ' [IDC_TREE_GRP1, tvCurSel IDC_TREE_GRP1, 1, true, [IDC_BT_1_GRP1, IDC_BT_2_GRP1, IDC_BT_3_GRP1]] call DMORBAT_fnc_removeFromGroup; '];
		...
		_ctrl ctrlSetEventHandler ["ButtonClick", ' [IDC_TREE_GRP1, tvCurSel IDC_TREE_GRP1, 1, true] spawn DMORBAT_fnc_editUnitAttributes; '];
		...
		_ctrl ctrlSetEventHandler ["ButtonClick", ' [IDC_TREE_GRP1, tvCurSel IDC_TREE_GRP1, 1, true] spawn DMORBAT_fnc_editUnitLoadout; '];
...
if (ctrlVisible IDC_GRP_TASK_GROUP2) then {
	_grp2Name = format ["Enemy %1", (_groupsData select 1) select 0];
	_ctrl = (_display displayCtrl IDC_TITLE_GROUP2);
	_ctrl ctrlSetText toUpper (_grp2Name);

	_ctrl = (_display displayCtrl IDC_TREE_GRP2);
	[IDC_TREE_GRP2, 2] call DMORBAT_fnc_updateCustomGroupsTreeList;
	_ctrl ctrlSetEventHandler ["TreeSelChanged", ' [IDC_TREE_GRP2, _this select 1, 2, true, [IDC_BT_1_GRP2, IDC_BT_2_GRP2, IDC_BT_3_GRP2]] call DMORBAT_fnc_TreeCustomGroup_selChanged; '];
	_ctrl ctrlShow true;
		...
		_ctrl ctrlSetText "Remove";
		_ctrl ctrlSetEventHandler ["ButtonClick", ' [IDC_TREE_GRP2, tvCurSel IDC_TREE_GRP2, 2, true, [IDC_BT_1_GRP2, IDC_BT_2_GRP2, IDC_BT_3_GRP2]] call DMORBAT_fnc_removeFromGroup; '];
		_ctrl ctrlSetTooltip "Removes the selected unit or group";
		_ctrl ctrlEnable false;
		...
		_ctrl ctrlSetText "Attributes";
		_ctrl ctrlSetEventHandler ["ButtonClick", ' [IDC_TREE_GRP2, tvCurSel IDC_TREE_GRP2, 2, true] spawn DMORBAT_fnc_editUnitAttributes; '];
		_ctrl ctrlSetTooltip "Modify the unit or group skill, probability of presence, etc.";
		_ctrl ctrlEnable false;
		...
		_ctrl ctrlSetText "Loadout";
		_ctrl ctrlSetEventHandler ["ButtonClick", ' [IDC_TREE_GRP2, tvCurSel IDC_TREE_GRP2, 2, true] spawn DMORBAT_fnc_editUnitLoadout; '];
		_ctrl ctrlSetTooltip "Edit the unit's loadout";
		_ctrl ctrlEnable false;
...
if (ctrlVisible IDC_GRP_TASK_GROUP3) then {
	_grp3Name = format ["Enemy %1", (_groupsData select 2) select 0];
	_ctrl = (_display displayCtrl IDC_TITLE_GROUP3);
	_ctrl ctrlSetText toUpper (_grp3Name);

	_ctrl = (_display displayCtrl IDC_TREE_GRP3);
	[IDC_TREE_GRP3, 3] call DMORBAT_fnc_updateCustomGroupsTreeList;
	_ctrl ctrlSetEventHandler ["TreeSelChanged", ' [IDC_TREE_GRP3, _this select 1, 3, true, [IDC_BT_1_GRP3, IDC_BT_2_GRP3, IDC_BT_3_GRP3]] call DMORBAT_fnc_TreeCustomGroup_selChanged; '];
	_ctrl ctrlShow true;
		...
		_ctrl ctrlSetText "Remove";
		_ctrl ctrlSetEventHandler ["ButtonClick", ' [IDC_TREE_GRP3, tvCurSel IDC_TREE_GRP3, 3, true, [IDC_BT_1_GRP3, IDC_BT_2_GRP3, IDC_BT_3_GRP3]] call DMORBAT_fnc_removeFromGroup; '];
		_ctrl ctrlSetTooltip "Removes the selected unit or group";
		_ctrl ctrlEnable false;
		...
		_ctrl ctrlSetText "Attributes";
		_ctrl ctrlSetEventHandler ["ButtonClick", ' [IDC_TREE_GRP3, tvCurSel IDC_TREE_GRP3, 3, true] spawn DMORBAT_fnc_editUnitAttributes; '];
		_ctrl ctrlSetTooltip "Modify the unit or group skill, probability of presence, etc.";
		_ctrl ctrlEnable false;
		...
		_ctrl ctrlSetText "Loadout";
		_ctrl ctrlSetEventHandler ["ButtonClick", ' [IDC_TREE_GRP3, tvCurSel IDC_TREE_GRP3, 3, true] spawn DMORBAT_fnc_editUnitLoadout; '];
		_ctrl ctrlSetTooltip "Edit the unit's loadout";
		_ctrl ctrlEnable false;
